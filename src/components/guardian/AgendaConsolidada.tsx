...
interface AgendaConsolidadaProps {
  allAulas: NextAula[];
  allEventos: NextEvento[];
...
const AgendaConsolidada = ({ 
  ...
  const currentWeekAulas = allAulas.filter(aula => 
  ...
  const renderCompactAulaItem = (aula: NextAula) => {
    ...
    return (
      <div 
        key={`aula-${aula.id}-${aula.crianca.id}`}
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0 space-y-1">
            {/* Top row: Child badge + school + type badge */}
            <div className="flex items-center gap-2 flex-wrap">
              <ChildBadge crianca={aula.crianca} />
  ...
  const renderCompactEventoItem = (evento: NextEvento) => {
    ...
    return (
      <div 
        key={`evento-${evento.id}-${evento.crianca.id}`}
        className={`p-3 rounded-lg bg-card border ${styles.border} ${
          isConfirmed ? 'bg-success/5' : isCanceled ? 'bg-destructive/5' : ''
        }`}
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0 space-y-1">
            {/* Top row: Child badge + school + type badge */}
            <div className="flex items-center gap-2 flex-wrap">
              <ChildBadge crianca={evento.crianca} />
              <span className="text-xs font-medium text-muted-foreground truncate">
      ...
      <Card className="border-primary/30 bg-gradient-to-br from-primary/5 to-transparent">
        <CardHeader className="pb-2 pt-3">
          ...
          <CardDescription className="text-xs">
            Todos os compromissos • Confirme até 15 min antes
          </CardDescription>
        </CardHeader>
      </Card>
...
export default AgendaConsolidada;
