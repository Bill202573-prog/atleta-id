...
import { MessageSquare, AlertTriangle, Info, Megaphone, Loader2, CheckCircle2, School, History, Users } from 'lucide-react';
import { toast } from 'sonner';
import { useState } from 'react';
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from '@/components/ui/collapsible';
import { ChevronDown } from 'lucide-react';

interface MuralConsolidadoProps {
  children: Array<{
    id: string;
    nome: string;
...
const MuralConsolidado = ({ children }: MuralConsolidadoProps) => {
  ...
  // Determine which child a comunicado is for
  const getChildForComunicado = (comunicado: ComunicadoEscolaComLeitura): { id: string; nome: string; foto_url: string | null } | null => {
    // If crianca_ids is set and matches one of our children
    if (comunicado.crianca_ids && comunicado.crianca_ids.length > 0) {
      for (const id of comunicado.crianca_ids) {
        const child = childrenMap.get(id);
        if (child) return child;
      }
    }
    // For general messages (not targeted), return null (will show all children badge)
    return null;
  };
  ...
  const renderComunicado = (comunicado: ComunicadoEscolaComLeitura, showConfirmButton: boolean) => {
    ...
    const targetChild = getChildForComunicado(comunicado);
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            {/* Child badge + school info */}
            <div className="flex items-center gap-2 flex-wrap mb-1">
              {targetChild ? (
                <ChildBadge crianca={targetChild} />
              ) : (
...
export default MuralConsolidado;
