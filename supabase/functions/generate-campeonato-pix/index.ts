...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (cadastroBancario?.asaas_api_key) {
      ...
      console.log("Using school's Asaas subconta API key (inline) for escola:", campeonato.escolinha_id, "Account ID:", cadastroBancario.asaas_account_id);
    } else {
      ...
      const { data: cadastroBancarioFull } = await supabase
        ...
        .eq('escolinha_id', campeonato.escolinha_id)
        .single();

      if (cadastroBancarioFull?.asaas_api_key) {
        activeApiKey = cadastroBancarioFull.asaas_api_key;
        console.log("Using school's Asaas subconta API key (direct query) for escola:", campeonato.escolinha_id, "Account ID:", cadastroBancarioFull.asaas_account_id);
      } else {
        console.warn('ALERTA: Escola sem API key Asaas configurada, usando master key. Escolinha ID:', campeonato.escolinha_id);
      }
    }
